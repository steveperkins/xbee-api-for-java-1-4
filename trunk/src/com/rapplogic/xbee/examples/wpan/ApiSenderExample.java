...
import com.rapplogic.xbee.api.SpecialByte;
	...
	private ApiSenderExample() throws Exception {
		...
		try {
			...
			while (true) {
				...
				// XBeeResponse response = xbee.sendTxRequest16(destination,
				// 0x0a, payload);

				// int[] payload = new int[] {1,2,3,4,5,6,7,8};
				// to verify correct byte escaping, we'll send a start byte
				int[] payload = new int[] { SpecialByte.START_BYTE.getValue()
						.intValue() };
				...
				while (true) {
					...
					if (response.getApiId() != ApiId.TX_STATUS_RESPONSE) {
						log
								.debug("expected tx status but received "
										+ response);
					} else {
						// log.debug("got tx status");

						if (((TxStatusResponse) response).getFrameId() != tx
								.getFrameId()) {
							throw new RuntimeException(
									"frame id does not match");
						}
						...
						if (((TxStatusResponse) response).getStatus() != TxStatusResponse.Status.SUCCESS) {
							...
							if (((TxStatusResponse) response).isAckError()) {
								ackErrors++;
							} else if (((TxStatusResponse) response)
									.isCcaError()) {
								ccaErrors++;
							...
							log
									.debug("Tx status failure with status: "
