...
public class IoSample {
	...
	public Boolean isDigitalOn(int pin) {
		...
		if (pin >= 0 && pin <= 7) {
			return new Boolean(ByteUtils.getBit(dioLsb.intValue(), pin + 1));
		} else {
			// pin 8
			return new Boolean(ByteUtils.getBit(dioMsb.intValue(), 1));		
		}
	}
		...
		if (parent.containsDigital()) {
			for (int i = 0; i <= 8; i++) {
				if (parent.isDigitalEnabled(i)) {
					Boolean ison = this.isDigitalOn(i);
					if(ison != null) {
						String msg = "";
