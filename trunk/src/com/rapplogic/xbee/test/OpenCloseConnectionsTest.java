...
public class OpenCloseConnectionsTest {
	...
	private final static Logger log = Logger
			.getLogger(OpenCloseConnectionsTest.class);
	...
	public OpenCloseConnectionsTest() throws XBeeException,
			InterruptedException {
		...
		// first connect directly to end device and configure. then comment out
		// configureXXX methods and connect to coordinator
		...
		if (!((AtCommandResponse) xbee.sendSynchronous(new AtCommand("CH")))
				.isOk())
		...
		try {
			...
			((AtCommandResponse) xbee.sendSynchronous(new AtCommand("CH")))
					.isOk();
			throw new RuntimeException("Should be disconnected");
		} catch (Exception e) {
			log.debug("Expected", e);
