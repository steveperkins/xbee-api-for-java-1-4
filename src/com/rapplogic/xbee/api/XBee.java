...
public class XBee implements IXBee {
	...
	public XBee() {
		this(new XBeeConfiguration().withMaxQueueSize(100).withStartupChecks(
				true));
	}
	...
	private void doStartupChecks() throws XBeeException {
		...
		try {
			...
			if (ap.getValue()[0] != 2) {
				log
						.warn("XBee radio is in API mode without escape characters (AP=1).  The radio must be configured in API mode with escape bytes (AP=2) for use with this library.");
				...
				if (ap.isOk()) {
					log
							.info("Successfully set AP mode to 2.  This setting will not persist a power cycle without the WR (write) command");
				} else {
			...
			if (vr.isOk()) {
				log.info("Firmware version is "
						+ ByteUtils.toBase16(vr.getValue()));
			}

			this.clearResponseQueue();
		} catch (XBeeTimeoutException ex) {
			throw new XBeeException(
					"AT command timed-out while attempt to set/read in API mode.  The XBee radio must be in API mode (AP=2) to use with this library");
		}
	}

	/**
	 * If XBeeConnection.startUpChecks is set to true (default), this method
	 * will check if the AP parameter is set correctly and attempt to update if
	 * AP=1. If AP=0 (Transparent mode), an exception will be thrown.
	 */
	...
	public static void registerResponseHandler(int apiId, Class clazz) {
	...
	/**
	 * Allows a protocol specific implementation of XBeeConnection to be used
	 * instead of the default RXTX connection. The connection must already be
	 * established as the interface has no means to do so.
	 */
	...
	public void sendRequest(XBeeRequest request) throws IOException {
		if (this.type != null) {
			...
			if (type == RadioType.SERIES1
					&& request.getClass().getPackage().getName().indexOf(
							"api.zigbee") > -1) {
				throw new IllegalArgumentException(
						"You are connected to a Series 1 radio but attempting to send Series 2 requests");
			} else if (type == RadioType.SERIES2
					&& request.getClass().getPackage().getName().indexOf(
							"api.wpan") > -1) {
				throw new IllegalArgumentException(
	...
	/**
	 * It's possible for packets to get interspersed if multiple threads send
	 * simultaneously. This method is not thread-safe because doing so would
	 * introduce a synchronized performance penalty for the vast majority of
	 * users that will not never need thread safety. That said, it is
	 * responsibility of the user to provide synchronization if multiple threads
	 * are sending.
	 * 
	 * Not thread safe.
	 * 
	...
	public void sendPacket(int[] packet) throws IOException {
		sendPacket(IntegerUtil.toArray(packet));
	}
	 ...
	 */
	public void sendPacket(Integer[] packet) throws IOException {
		// TODO should we synchronize on read lock so we are sending/recv.
		// simultaneously?
		...
		if (log.isInfoEnabled()) {
			log.info("sending packet to XBee "
					+ ByteUtils.toBase16(IntegerUtil.toArray(packet)));
		}

		for (int i = 0; i < packet.length; i++) {
			int aPacket = packet[i].intValue();
	...
	/**
	 * Sends an XBeeRequest though the XBee interface in an asynchronous manner,
	 * such that it will return immediately, without waiting for a response.
	 ...
	 * <p/>
	 * This method returns the first response object with a matching frame id,
	 * within the timeout period, so it is important to use a unique frame id
	 * (relative to previous subsequent requests).
	 * <p/>
	 ...
	 * <p/>
	 * Keep in mind responses received here will also be available through the
	 * getResponse method and the packet listener. If you would prefer to not
	 * have these responses added to the response queue, you can add a
	 * ResponseQueueFilter via XBeeConfiguration to ignore packets that are sent
	 * in response to a request. Another alternative is to call
	 * clearResponseQueue prior to calling this method.
	 * <p/>
	 * It is recommended to use a timeout of at least 5 seconds, since some
	 * responses can take a few seconds or more (e.g. if remote radio is not
	 * powered on).
	 * <p/>
	 ...
	 *             thrown if no matching response is identified
	 */
	public XBeeResponse sendSynchronous(final XBeeRequest xbeeRequest,
		...
		try {
			...
			// this makes it thread safe -- prevents multiple threads from
			// writing to output stream simultaneously
			...
			pl = new PacketListener() {
				...
				public void processResponse(XBeeResponse response) {
					if (response instanceof XBeeFrameIdResponse
							&& ((XBeeFrameIdResponse) response).getFrameId() == xbeeRequest
									.getFrameId()) {
	...
	/**
	 * Uses sendSynchronous timeout defined in XBeeConfiguration (default is
	 * 5000ms)
	 */
	...
	/**
	 * Same as getResponse(int) but does not timeout. It's highly recommend that
	 * you always use a timeout because if the serial connection dies under
	 * certain conditions, you will end up waiting forever!
	 * <p/>
	 * Consider using the PacketListener for asynchronous (non-blocking)
	 * behavior
	 * 
	 * @return
	 * @throws XBeeException
	 ...
	 * 2. An XBeeTimeoutException is thrown (i.e. queue was empty for duration
	 * of timeout) <br/>
	 * 3. Null is returned if timeout is 0 and queue is empty. <br/>
	 * <p/>
	 * 
	 * @param timeout
	 *            milliseconds to wait for a response. A value of zero disables
	 *            the timeout
	 * @return
	 * @throws XBeeException
	 * @throws XBeeTimeoutException
	 *             if timeout occurs before a response is received
	 */
	public XBeeResponse getResponse(int timeout) throws XBeeException,
			XBeeTimeoutException {
		return this.getResponseTimeout(new Integer(timeout));
	}
	...
	private XBeeResponse getResponseTimeout(Integer timeout)
		...
		try {
			if (timeout != null) {
				response = (XBeeResponse) parser.getResponseQueue().poll(
						timeout.longValue(), TimeUnit.MILLISECONDS);
			} else {
				response = (XBeeResponse) parser.getResponseQueue().take();
			}
		} catch (InterruptedException e) {
			throw new XBeeException(
					"Error while attempting to remove packet from queue", e);
		}

		if (response == null && timeout.intValue() > 0) {
			throw new XBeeTimeoutException();
		}
	...
	// public List<? extends XBeeResponse> collectResponses(int wait,
	// ResponseFilter filter, CollectTerminator terminator) throws XBeeException
	// {
	//
	// }

	/**
	 * Collects responses until the timeout is reached or the CollectTerminator
	 * returns true
	 * 
	 * @param wait
	 * @param terminator
	 ...
	 */
	// <? extends XBeeResponse>
	public List collectResponses(int wait, CollectTerminator terminator)
			throws XBeeException {
		...
		log.debug("Time is up.. returning list with " + responseList.size()
				+ " packets");
	 ...
	 */
	// <? extends XBeeResponse>
	public List collectResponses(int wait) throws XBeeException {
		return this.collectResponses(wait, null);
	}

	/**
	 * Returns the number of packets available in the response queue for
	...
	public void close() {
		...
		if (parser != null) {
			parser.setDone(true);
			// interrupts thread, if waiting. does not interrupt thread if
			// blocking on read
	...
	/**
	 * Indicates if serial port connection has been established. The open method
	 * may be called if true it returned
	 * 
	 * @return
	 */
	public boolean isConnected() {
		try {
			if (parser.getXBeeConnection().getInputStream() != null
					&& parser.getXBeeConnection().getOutputStream() != null) {
				return true;
			}
	 ...
	 * It will return frame ids in a sequential manner until the maximum is
	 * reached (0xff) and it flips to 1 and starts over.
	 * 
	 * Not Thread-safe
	 * 
	 * @return
