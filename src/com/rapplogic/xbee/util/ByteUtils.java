...
public class ByteUtils {
	...
	/**
	 * There is a slight problem with this method that you might have noticed; a
	 * Java int is signed, so we can't make use of the 32nd bit. This means this
	 * method does not support a four byte value with msb greater than 01111111
	 * ((2^7-1) or 127) and will throw a runtime exception if it encounters this
	 * situation.
	 * 
	 * TODO use long instead of int to support 4 bytes values. note that long
	...
	public static int convertMultiByteToInt(int[] bytes) {
		...
		for (int i = 0; i < bytes.length; i++) {
			...
			if (bytes[i] > 0xFF) {
				throw new IllegalArgumentException(
						"Values exceeds byte range: " + bytes[i]);
			}
	...
	/**
	 * Converts an int array to string. Note: this method does not validate that
	 * int values map to valid characters
	 * 
	 * @param arr
	 * @return
	 */
	...
	/**
	 * Returns true if the bit is on (1) at the specified position Position
	 * range: 1-8
	 */
	public static boolean getBit(int b, int position) {
		...
		if (b > 0xff) {
			throw new IllegalArgumentException("input value [" + b
	...
	public static String formatByte(int b) {
		return "base10=" + Integer.toString(b) + ",base16=" + toBase16(b)
				+ ",base2=" + toBase2(b);
	}
	...
	public static int parse10BitAnalog(IIntInputStream in, int pos)
			throws IOException {
